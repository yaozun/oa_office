<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ page import="com.oa_office.user.pojo.User"%>
<!DOCTYPE HTML>
<html>
<head>
<link rel="icon" href="${pageContext.request.contextPath}/images/index.ico" type="image/x-icon">
...
<script id="microloader" data-app="b84250be-db47-4bf4-b9b4-7f398e303762"
	type="text/javascript">
	var Ext = Ext || {};
	Ext.manifest = Ext.manifest || "classic.json";// @tag core
	// @define Ext.Boot
	...
	Ext.Boot = Ext.Boot
			|| (function(emptyFn) {
				...
				_debug = function(message) {
					...
					var lowerCase = false, obj = Array.prototype.shift
							.call(arguments), index, i, len, value;
					...
					return obj;
				}, _getKeys = (typeof Object.keys == 'function') ? function(
						object) {
					if (!object) {
						return [];
					}
				...
				Boot = {
					...
					browserPriority : [ 'edge', 'opera', 'dolfin',
							'webosbrowser', 'silk', 'chromeiOS',
							'chromeMobile', 'ie', 'firefox', 'safari', 'chrome' ],
					...
					detectPlatformTags : function() {
						var me = this, ua = navigator.userAgent, isMobile = /Mobile(\/|\s)/
								.test(ua), element = document
								.createElement('div'), isEventSupported = function(
								name, tag) {
							if (tag === undefined) {
								tag = window;
							}
						...
						getBrowsers = function() {
							...
							for (index = 0; index < len; index++) {
								...
								if (!matched) {
									...
									match = ua.match(new RegExp('(' + value
											+ ')([\\w\\._]+)'));
									version = match && match.length > 1 ? parseInt(match[2])
											: 0;
									if (version) {
										matched = true;
									}
							...
							for (index = 8; index <= maxIEVersion; ++index) {
								prefix = 'ie' + index;
								browsers[prefix + 'm'] = version ? version <= index
										: 0;
								browsers[prefix] = version ? version === index
										: 0;
								browsers[prefix + 'p'] = version ? version >= index
										: 0;
							}
						...
						getOperatingSystems = function() {
							...
							for (index = 0, activeCount = 0; index < len; index++) {
								...
								if (matched
										&& (matched === 'HTC_' || matched === 'Silk/')) {
									version = 2.3;
								} else {
									version = match && match.length > 1 ? parseFloat(match[match.length - 1])
											: 0;
								}
						...
						_merge(_tags, browsers, systems, devices,
								platformParams, true);
						...
						_tags.tablet = !!(!_tags.phone && (_tags.ipad
								|| _tags.android || _tags.silk
								|| _tags.rimtablet || (_tags.ie10 && /; Touch/
								.test(ua))));
					...
					filterPlatform : function(platform, excludes) {
						...
						var plen = platform.length, elen = excludes.length, include = (!plen && elen), // default true if only excludes specified
						i, tag;
					...
					init : function() {
						...
						Boot.isIE8 = Boot.hasReadyState && !Boot.hasAsync
								&& Boot.hasDefer && !Boot.hasOnLoad;
						Boot.isIE9 = Boot.hasReadyState && !Boot.hasAsync
								&& Boot.hasDefer && Boot.hasOnLoad;
						Boot.isIE10p = Boot.hasReadyState && Boot.hasAsync
								&& Boot.hasDefer && Boot.hasOnLoad;
						...
						if (Boot.isIE8) {
							...
							Boot.isIE10 = (new Function(
									'/*@cc_on return @_jscript_version @*/')()) === 10;
							Boot.isIE10m = Boot.isIE10 || Boot.isIE9
									|| Boot.isIE8;
						}
						...
						Boot.baseUrl = baseUrl.substring(0, baseUrl
								.lastIndexOf('/') + 1);
					...
					canonicalUrl : function(url) {
						...
						var ret = resolverEl.href, dc = _config.disableCachingParam, pos = dc ? ret
								.indexOf(dc + '=')
								: -1, c, end;
					...
					requestComplete : function(request) {
						...
						if (!Boot.currentRequest
								&& Boot.suspendedQueue.length == 0) {
							Boot.fireListeners();
						}
					},

					isLoading : function() {
						return !Boot.currentRequest
								&& Boot.suspendedQueue.length == 0;
					},

					fireListeners : function() {
						var listener;
						while (Boot.isLoading()
								&& (listener = Boot.listeners.shift())) {
							listener();
						}
					},
					...
					getPathsFromIndexes : function(indexMap, loadOrder) {
						...
						return Request.prototype.getPathsFromIndexes(indexMap,
								loadOrder);
					},
				...
				Request.prototype = {
					...
					getLoadIndexes : function(item, indexMap, loadOrder,
						...
						while (item = queue.shift()) {
							...
							if (item.canonicalPath) {
								...
								entry = Boot.getEntry(this
										.prepareUrl(item.path));
							}
					...
					expandUrl : function(url, loadOrder, loadOrderMap,
							indexMap, includeUses, skipLoaded) {
						var item, resolved;
						...
						if (loadOrder) {
							...
							if (item) {
								resolved = this.getLoadIndexes(item, indexMap,
										loadOrder, includeUses, skipLoaded);
					...
					expandUrls : function(urls, includeUses) {
						...
						for (i = 0, len = urls.length; i < len; i++) {
							...
							tmpExpanded = this.expandUrl(urls[i], loadOrder,
									loadOrderMap, indexMap, includeUses, false);
					...
					notify : function() {
						var me = this;
						if (!me.done) {
							var error = me.error, fn = me[error ? 'failure'
									: 'success'], delay = ('delay' in me) ? me.delay
					...
					onDone : function(listener) {
						var me = this, listeners = me.listeners
								|| (me.listeners = []);
						if (me.done) {
							listener(me);
						} else {
				...
				function Entry(cfg) {
					...
					var charset = cfg.charset || Boot.config.charset, manifest = Ext.manifest, loader = manifest
							&& manifest.loader, cache = (cfg.cache !== undefined) ? cfg.cache
				...
				Entry.prototype = {
					...
					isCrossDomain : function() {
						...
						if (me.crossDomain === undefined) {
							...
							me.crossDomain = (me.getLoadUrl().indexOf(
									Boot.origin) !== 0);
						}
					...
					isCss : function() {
						...
						if (me.css === undefined) {
							if (me.url) {
								...
								me.css = assetConfig ? assetConfig.type === "css"
										: cssRe.test(me.url);
							} else {
								me.css = false;
							}
					...
					getLoadUrl : function() {
						...
						url = me.canonicalPath ? me.url : Boot
								.canonicalUrl(me.url);

						if (!me.loadUrl) {
							me.loadUrl = !!me.buster ? (url
									+ (url.indexOf('?') === -1 ? '?' : '&') + me.buster)
									: url;
						}
						return me.loadUrl;
					},
					...
					onContentLoaded : function(response) {
						...
						if ((exception || status === 0)
							...
							me.error = ("Failed loading synchronously via XHR: '"
									+ url
									+ "'. It's likely that the file is either being loaded from a "
									+ "different domain or from the local file system where cross "
									+ "origin requests are not allowed for security reasons. Try " + "asynchronous loading instead.") || true;
							me.evaluated = true;
						} else if ((status >= 200 && status < 300)
								|| status === 304 || _environment.phantom
								|| (status === 0 && content.length > 0)) {
							me.content = content;
						} else {
							me.error = ("Failed loading synchronously via XHR: '"
									+ url
									+ "'. Please verify that the file exists. XHR status code: " + status) || true;
							me.evaluated = true;
						}
					},
				...
				Ext.disableCacheBuster = function(disable, path) {
					var date = new Date();
					date.setTime(date.getTime() + (disable ? 10 * 365 : -1)
							* 24 * 60 * 60 * 1000);
					date = date.toGMTString();
					doc.cookie = 'ext-cache=1; expires=' + date + '; path='
							+ (path || '/');
				};
	...
	Ext.Microloader = Ext.Microloader
			|| (function() {
				var Boot = Ext.Boot, _debug = function(message) {
					//console.log(message);
					...
					setAsset : function(key, content) {
						try {
							...
							if (_storage && e.code == e.QUOTA_EXCEEDED_ERR) {
								_warn("LocalStorage Quota exceeded, cannot store "
										+ key + " locally");
							}
						}
					}
				...
				var Asset = function(cfg) {
					...
					this.type = cfg.type;
					this.key = getStorageKey(this.assetConfig.path,
				...
				Asset.prototype = {
					shouldCache : function() {
						return _storage && this.assetConfig.update
								&& this.assetConfig.hash
								&& !this.assetConfig.remote;
					},

					is : function(asset) {
						return (!!asset && this.assetConfig
								&& asset.assetConfig && (this.assetConfig.hash === asset.assetConfig.hash))
					},

					cache : function(content) {
						if (this.shouldCache()) {
							LocalStorage.setAsset(this.key, content
									|| this.content);
						}
					},
				...
				var Manifest = function(cfg) {
					...
					this.deltas = this.content.cache ? this.content.cache.deltas
							: null;
					this.cacheEnabled = this.content.cache ? this.content.cache.enable
							: false;
				...
				Manifest.prototype = {
					...
					cache : function(content) {
						if (this.shouldCache()) {
							LocalStorage.setAsset(this.key, JSON
					...
					is : function(manifest) {
						_debug("Testing Manifest: " + this.hash + " VS "
								+ manifest.hash);
				...
				var _listeners = [], _loaded = false, Microloader = {
					init : function() {
						...
						var microloaderElement = document
					...
					applyCacheBuster : function(url) {
						var tstamp = new Date().getTime(), sep = url
								.indexOf('?') === -1 ? '?' : '&', progressive = Ext.manifest.progressive, serviceWorker = progressive
					...
					run : function() {
						...
						if (typeof manifest === "string") {
							var extension = ".json", url = manifest
									.indexOf(extension) === manifest.length
									- extension.length ? manifest : manifest
									+ ".json", key = getStorageKey(url), content = LocalStorage
									.retrieveAsset(key);
							...
							if (content) {
								...
								if (location.href.indexOf('file:/') === 0) {
									Manifest.url = Microloader
											.applyCacheBuster(url + 'p');
					...
					load : function(manifest) {
						...
						for (len = assets.length, i = 0; i < len; i++) {
							...
							if (include) {
								...
								Boot.assetConfig[asset.assetConfig.path] = Boot
										.apply({
											type : asset.type
										}, asset.assetConfig);
							}
						...
						if (cachedAssets.length > 0) {
							...
							while (cachedAssets.length > 0) {
								...
								_debug("Preloading/Fetching Cached Assets from: "
										+ asset.assetConfig.path);
								Boot.fetch(asset.assetConfig.path, (function(
										asset) {
					...
					onCachedAssetLoaded : function(asset, result) {
						...
						if (!result.error) {
							...
							_debug("Checksum for Cached Asset: "
									+ asset.assetConfig.path + " is "
									+ checksum);
							Boot.registerContent(asset.assetConfig.path,
									asset.type, result.content);
					...
					patch : function(content, delta) {
						...
						for (i = 0, ln = delta.length; i < ln; i++) {
							...
							if (typeof chunk === 'number') {
								output.push(content.substring(chunk, chunk
										+ delta[++i]));
							} else {
								output.push(chunk);
							}
					...
					checkForAppCacheUpdate : function() {
						...
						if (_cache.status === _cache.UPDATEREADY
							...
							_cache.addEventListener('error',
									Microloader.onAppCacheError);
					...
					checkForUpdates : function() {
						...
						_debug("Checking for updates at: "
								+ Microloader.manifest.url);
						Boot.fetch(Microloader
								.applyCacheBuster(Microloader.manifest.url),
					...
					filterAsset : function(asset) {
						...
						if (cfg.platform || cfg.exclude) {
							return Boot.filterPlatform(cfg.platform,
									cfg.exclude);
						}
						return true;
					},
					...
					onUpdatedManifestLoaded : function(result) {
						...
						if (!result.error) {
							...
							if (!Microloader.manifest.is(manifest)) {
								...
								currentAssets = Microloader.manifest
										.getAssets();
					...
					onFullAssetUpdateLoaded : function(asset, result) {
						...
						if (!result.error) {
							checksum = Microloader.checksum(result.content,
									asset.assetConfig.hash);
							_debug("Checksum for Full asset: "
									+ asset.assetConfig.path + " is "
									+ checksum);
							if (!checksum) {
					...
					onDeltaAssetUpdateLoaded : function(asset, oldAsset, result) {
						...
						if (!result.error) {
							...
							try {
								...
								content = Microloader.patch(oldAsset.content,
										json);
							...
							_warn("Error loading delta patch for "
									+ asset.assetConfig.path
									+ " with hash "
									+ oldAsset.assetConfig.hash
									+ " . This asset will be uncached for future loading");
					...
					parseResult : function(result) {
						var rst = {};
						if ((result.exception || result.status === 0)
								&& !Boot.env.phantom) {
							rst.error = true;
